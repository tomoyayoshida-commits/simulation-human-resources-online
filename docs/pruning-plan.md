# 最適化の枝刈り方針

枝刈りの具体手順を提案する。**アルゴリズム変更（§9で事前確認必須）**なので、実装前の合意用として書く。

## 方針：branch-and-bound（上界による探索打ち切り）

配分候補そのものを機械的に絞る（危険）のではなく、**各候補をMCMFで解く前に「最良候補を超えられない」と証明できたら解かずに捨てる**。安全性の根拠は「緩和問題の最適値は本問題の最適値以上＝有効な上界」であること。

### 前提（安全性の要）

1. MCMFが最大化する `Σ value(i,Xi)` は、`primaryMetric*1e6 + companyRevenue`（＝§7-1の辞書式スカラー）とアグリゲートで一致する（`buildValues`の合成そのもの）。
2. 外側の`isBetter`（primary→secondary→nA→nB）は、この合成スカラーと**単調**（`LEX_WEIGHT=1e6`がsecondaryの逆転を許さない設計のため）。
   → 合成スカラーの上界で枝刈りすれば、`isBetter`基準の最適解を落とさない。
3. **候補の反復順は結果に影響しない**（`isBetter`がnA/nBまで含む全順序で勝者を決めるため、iteration orderは無関係）。よって並べ替えは自由。

### 手順

**① 貢献度の事前計算（1回）**
`contribution(e,unit)` は配分に依存しないので全社員×3事業部を先に計算してキャッシュ。`revValue = (BASE*GROWTH/100 * eff[unit]) * contribution` と分離できる。

**② 候補ごとの上界 UB（MCMFの前、O(n) 程度）**
- 現行どおり `eff` と `buildValues`（＝各社員×事業部の合成値）を計算。
- **各事業部Xについて、合成値 `value(i,X)` の上位 `counts[X]` 件を単純合計**し、3事業部で足す。
- 「1社員は1事業部」という排他制約を外した緩和なので、実MCMF最適値 ≤ UB（有効な上界）。
- コストは選択（quickselect/部分ソート）でO(n)〜O(n log n)。MCMFの `N×V²` に比べ無視できる。

**③ 最良優先＋早期打ち切り**
- 全候補のUBを計算 → **UB降順にソート**して反復。
- 各候補で：`best !== null` かつ `UB < objectiveScalar(best) − EPS` なら、**以降の候補は全てUBが小さいので break**（枝刈りではなく探索終了）。
- そうでなければ従来どおり `solveAssignment → computeSimulationResult → isBetter` で更新。
- `objectiveScalar(result,task)` は`buildValues`と同じ基底で丸め後値から算出：
  - 課題1: `companyRevenue*1e6`
  - 課題2: `A.profit*1e6 + companyRevenue`
  - 課題3/4: `B|C.finalRevenue*1e6 + companyRevenue`

**④ 実行不能フォールバック（closest）の扱い**
- `closest`（全社売上最大）は `best===null` のときだけ使われる。
- **打ち切りは `best!==null` のときのみ発生**させる → `best` を返す局面なので `closest` は不使用、不整合は起きない。
- 全候補が実行不能な稀ケースは `best===null` のまま最後まで解く（＝この場合だけ高速化なし、正当性優先）。UB降順なので実行可能候補は通常すぐ見つかり `best` が立つ。

### EPS（丸め誤差マージン）

- 候補側UBは丸め前、`objectiveScalar(best)`は丸め後 → 差分を吸収する必要。
- 丸めによるprimaryの上振れ ≲ 0.02億 → スカラーで `0.02×1e6 = 2e4`。secondary上振れ ≲ 0.03。
- **`EPS = 1e5`（primary換算0.1億、実誤差の約5倍）を提案**。大きめでも「枝刈りが弱まるだけで誤答は起きない」側なので安全。緩和ギャップ（通常数億×1e6）に対しEPSは十分小さく、枝刈り効果はほぼ落ちない。

### 期待効果

- 最良優先で強い暫定解を即獲得 → 大多数の候補をbreakで飛ばせる。約10秒 → 体感数秒を見込む（確約はしない）。

### 検証計画（§9準拠・実装の必須ゲート）

1. 新旧実装を並走させ、**実データ `human_resources_100.csv` の4課題で配置・売上・利益が完全一致**を確認。
2. ランダム社員データ（採用後110名含む）で旧実装と全一致するプロパティテストを追加。
3. `npm test` 全通過後に差し替え。

---

## 追記（2026-08-28）②UBのquickselect化：実測のうえ見送りを推奨

`upperBoundRawTotal` は今も全ソート（`docs/refactor-plan.md` C-2）。着手可否を判断するため実測した。

**実測（実データ100名・4課題）**

| 項目 | 値 |
|---|---|
| 4課題合計 | 1126ms（課題1 315 / 2 91 / 3 128 / 4 592） |
| 候補数 | 861 |
| 上界の全ソート（861候補×3事業部×4課題） | 約117ms＝**全体の10.4%** |
| ソートを完全に省いた場合（理論下限） | 削減幅の上限 **68ms** |

CPUプロファイルの「`upperBoundRawTotal` 11%」と整合する。ただし削減の上限が68msで、
quickselect自体にも選択コストがあるため実際の削減は**40〜50ms程度＝全体の約4%**にとどまる。
支配的なのは課題4の592ms（MCMF）であって上界計算ではない。

**さらに、そのままでは結果が変わりうる。**
現在の実装は降順ソート後に上位k件を先頭から加算しており、和は**加算順に依存する**。
quickselectは同じ集合を別の順で返すため `ub` の下位ビットが変わる。
`PRUNE_EPS = 1e5` に対して誤差は無視できる大きさだが、`prepared.sort((a,b) => b.ub - a.ub)` の
並び順が僅差の候補間で入れ替わりうる。最終結果は `isBetter` と `closest` のタイブレークで
順序非依存に保たれている設計だが、**「アルゴリズムを触るなら §9 の再合意が要る」水準の変更**である。
和をビット一致させるには選択後の上位k件を結局ソートすることになり、削減幅はさらに縮む。

**結論：費用対効果が見合わないため見送りを推奨する。**

**代替案（ソートのループ外への巻き上げ）は実装済み — 下の「実装結果」を参照**
`buildValues` の値は、事業部 `u` について
`perUnit[u] = K_u × eff_u × contrib_u`（`K_u = baseRevenue_u × growth_u / 100`）の形になる。
`eff_u > 0` なので、**事業部内での社員の順位は `eff_u`（＝人数配分）に依存せず `contrib_u` だけで決まる**。
つまり課題1の全事業部と、課題2〜4の目的外事業部については、
ソートを候補ループの外に出して1回だけ行い、各候補は上位k件の**プレフィックス和**を引くだけでよい
（累積和を持てば O(1)）。861回のソートが1回になるので、quickselectの定数倍改善より効く。
※課題2のA事業部だけは `primary = rev − cost` にコスト項が入り、順位が `eff_A` に依存するため対象外。

### 実装結果（2026-08-28・`optimizer.buildValueOrders`）

**加算順を変えずに実装できたため、結果はビット単位で不変。** §9の「結果が変わる変更」には当たらない。

並べ替えの*結果*だけを使い回し、加算する値も加算順もそのままにした。安定ソートなので
値そのものを降順ソートした場合と同じ置換になる（非負倍は順序を保ち、同値のタイブレークも
入力順で一致。係数が0なら全値0で和も0）。

| 検証 | 結果 |
|---|---|
| ub のビット一致（実データ4課題×861候補／同点を量産した乱数3種／growth=0の縮退） | 7680件**全件一致** |
| 実データ4課題の SHA256 | `d0a98405…` のまま |
| `npm test` | 65件 pass |

**効果**（JITウォームアップ後の3回中央値）

| | 合計 | 課題1 | 課題2 | 課題3 | 課題4 |
|---|---|---|---|---|---|
| 巻き上げ前 | 883ms | 219 | 73 | 87 | 504 |
| 巻き上げ後 | **837ms** | 194 | 67 | 71 | 505 |

46ms（5.2%）短縮。課題4はMCMFが支配的なので変わらない。

### テストの落とし穴（重要）

最初は「同点だらけのデータで総当たり実装と一致する」テストを書いたが、
**巻き上げの前提をわざと壊しても全件パスした**。上界が不正になっても、
枝刈りがたまたま真の最適解を切り落とさなければ結果は一致してしまうためである。

そこで上界そのものを突き合わせる `upperBoundsForCandidates`（テスト専用の観測点）を
`optimizer.ts` から公開し、基準実装との**ビット一致**を検証するテストを追加した。
前提を壊すと `task=2 候補0` で差 `-2,540,998.94` を出して落ちることを確認済み。
**この巻き上げを触るときは、総当たり比較ではなくこのテストが唯一の守りになる。**
