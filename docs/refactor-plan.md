...
- 検証に使った Electron E2E は、**恒久的な回帰テストとして `test/e2e/run.mjs` に復元済み**（`npm run test:e2e`・21項目）。
  ...
  また Electron 37 以降の `console-message` は details オブジェクト1つを渡す新シグネチャで、
  一時スクリプトの `(e, level, message)` 前提ではエラーを取りこぼす状態だった。両シグネチャを受けるよう直し、
  「コンソール監視が1件以上受信していること」自体を検査項目に入れて空振りを防いでいる。
