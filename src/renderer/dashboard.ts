...
export function renderDashboard(
  ...
  if (bars) {
    bars.innerHTML = UNIT_IDS.map((u) => {
      ...
      return `<div class="bar-row"><span class="label">${UNIT_LABEL[u]}</span><div class="bar-track"><div class="bar-fill" style="width:${w.toFixed(1)}%;background:${UNIT_VAR[u]};"></div></div><span class="val">${headcount[u]}名（適正${OPTIMAL_HEADCOUNT[u]}）</span></div>`
    }).join('')
  }
  ...
  if (constraint) {
    const revDiff = round2(result.companyRevenue - PREV_YEAR_REVENUE)
    ...
    for (const u of UNIT_IDS) {
      ...
      html += `<tr><td>${UNIT_LABEL[u]} 最低人数</td><td>${MIN_HEADCOUNT[u]}名以上</td><td>${headcount[u]}名</td><td style="color:${color};">${margin >= 0 ? '+' : ''}${margin}名${warn ? '（注意）' : ''}</td><td>${ok ? pill('good', '● 満たす') : pill('crit', '● 未達')}</td></tr>`
    }
