...
## 2. 何を作っているか
社員をA/B/C事業部へ配置し売上・利益を最適化するアプリ。Web(SPA)+TypeScript+Firebase。
取込は**上限方式**で社員データ最大200名・追加採用最大20名（2026-09-07 合意。課題原文の100名/10名もそのまま通る）。
...
| `npm test` | node:test全197件・約23秒（2026-09-07 実測。旧「167件・37〜45秒」は割当の二分ヒープ化前の値） |
...
適正・最低人数は**名簿100名のときの値**。他の人数では `constants.standardParamsFor(人数)` が比例配分する
（200名ならA80/B70/C50・A60/B40/C20。100名では `DEFAULT_PARAMS` と完全一致）。全社売上下限58億は人数で変えない。
...
4. 採用後も適正人数は**採用前の人数**を基準に据え置く（過剰ペナルティに入りやすいのは意図的）。
   `#p5` は `setStandardHeadcount(採用前の人数)` を呼ぶ＝採用ぶんを基準に含めない
...
- 最適化速度は解決済み（2026-09-07 再実測）：実データ100名は4課題で約0.55秒、8通り先読みで約1.30秒。200名（100名を複製）でも8通り5.57秒。`npm test`の約23秒は枝刈り無し基準実装のテスト1本が大半を占める。詳細`docs/pruning-plan.md`
- **200名級で効くのはメモリ**：`runOptimization`は`buildValues`を`eff`ごとにキャッシュしている。候補ごとに作り直すと200名でheapTotalが540MB〜1GBに達しブラウザで落ちる。`eff`は補正表の段階値の積なので事業部あたり高々8通りしか無い、という前提に依存している
- `assignment.ts`のDijkstraは二分ヒープ。取り出し順を「dist最小→ノード番号最小」の辞書式に揃えてあり、これを崩すと同値の増加路の選び方が変わって配置結果が動く（`npm run snapshot`が唯一の守り）
